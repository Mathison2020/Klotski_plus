...
- **局面编码**：关卡编辑器可将当前草稿导出为 `KLP1.` 编码，也可解析编码并载入编辑；保存动作仍由用户单独确认
- **编辑器验证**：可把当前棋盘锁定为复位点，并在后台判断局面是否有解及其最短步数
- **异形预设**：“缺月重围”仅留两格空位，最短解 59 步且必须旋转 3/4 圆

## 局面编码格式

`KLP1.<payload>` 由格式版本前缀和 URL-safe Base64 载荷组成。载荷为 UTF-8 JSON，包含格式标识、版本、关卡名称以及自描述的棋块数组。每个棋块保留 `type`、坐标、朝向及附加属性；以后增加棋块类型或属性时可以沿用 KLP1 外层格式，不兼容升级则使用新的版本号。导入时会校验版本、棋块类型、边界、重叠、曹操数量与自由空间。
